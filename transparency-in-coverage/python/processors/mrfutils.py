...
def _write_table_to_db(rows, tablename, cnx):
    fieldnames = SCHEMA[tablename]

    if type(rows) == list:
        for row in rows:
            _write_table_to_db(row, tablename, cnx)
...
def write_in_network_item(
    ...
    if cnx is not None:
        _write_table_to_db(code_row, 'codes', cnx)
    ...
    for rate in in_network_item['negotiated_rates']:
        ...
        if cnx is not None:
            _write_table_to_db(prices_providr_group_rows, 'prices_provider_groups', cnx)
...
def write_plan(
    ...
    if cnx is not None:
        _write_table_to_db(plan_file_row, 'plans_files', cnx)
